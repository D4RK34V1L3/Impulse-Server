	...
	viewlogs: function (target, room, user) {
		...
		fs.readFile('logs/chat/' + toId(targetRoom) + '/' + splitDate[0] + '-' + splitDate[1] + '/' + date + '.txt', 'utf8', (err, data) => {
			...
			fs.writeFile('static/logs/' + filename, data, err => {
				...
				this.sendReply(
					"|raw|You can view the logs at <a href=\"https://newworkimpulse-lustyash.c9users.io:" + Config.port +
					"/logs/logviewer.html?file=" + filename + "\">https://newworkimpulse-lustyash.c9users.io:" + Config.port +
